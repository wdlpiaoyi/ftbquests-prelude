...
- **Third-party buttons that only work against a server are hidden in the local book.** `rs_integration`
  ...
  panel); both drive server-side work that cannot do anything outside a world. The prefixes of the
  classes to hide live in the new `hiddenButtonPackages` config option, so another one can be hidden -
  or one un-hidden - without a new release. Removal happens in the panels' layout pass, so it covers
  buttons added by mods with a higher mixin priority.

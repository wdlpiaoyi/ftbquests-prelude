...
- **模组群系只有在本次会话进过一次世界后才会出现在群系选择器里。** 生物群系是服务端同步的动态
  注册表，主菜单下客户端根本拿不到；原版群系任何时候都能列出。
- **流体选择器在主菜单下没有图标。** FTB Library 从方块图集里取流体贴图，而图集要进过世界才有；
  本机未经修改的 FTB Quests 表现一致，所以不做特殊处理。
- **`T` / `S` / `D`**（编辑任务标题/副标题/描述）与维度任务图标在本地任务书无效。本机未经修改的
  FTB Quests 表现一致，原因未知，不做特殊处理。
